  ...
  reducers: {
    ...
    removeAssignment: (state, action) => {
      ...
      if (state[classId] && state[classId][topicName]) {
        state[classId][topicName].assignments = state[classId][topicName].assignments.filter(
    ...
    addSubmission: (state, action) => {
      ...
      Object.keys(topics).forEach((topicName) => {
        ...
        if (assignmentIndex >= 0) {
          ...
          console.log(`Submission added to assignment ${assignmentId} in topic ${topicName}`);
        } else {
          console.log(`Assignment with ID ${assignmentId} not found in topic ${topicName}.`);
        }
      });
    },
        ...
        if (assignmentIndex >= 0) {
          ...
          if (Array.isArray(assignment.submissions)) {
            ...
            if (submissionIndex >= 0) {
              ...
              console.log(`Updated grade for submission ${submissionId} to ${grade}`);
            } else {
              console.log(`Submission with ID ${submissionId} not found in assignment ${assignmentId}`);
            }
          } else {
